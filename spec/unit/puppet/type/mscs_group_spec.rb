...
describe Puppet::Type.type(:mscs_group) do

  it do
    expect {
      Puppet::Type.type(:mscs_group).new(:name => '')
  ...
  'clustername',
    ...
    context "when a valid cluster parameter #{param} => is specified" do
      it "should not raise an error" do
        expect {
          Puppet::Type.type(:mscs_group).new(:name => 'test', param => "blah")
          }.to_not raise_error
      end
    end
  end
  ...
  '123',
    ...
    context "when param used is #{param}" do
      it "should fail if the resourcetype #{param} is not valid" do
        expect {
          Puppet::Type.type(:mscs_group).new(:name => 'test', param => 'C:\windows')
          }.to raise_error
      end
    end
  end
